...
const ConfirmedTasks = () => {
    ...
    const handleOrder = async (taskId) => {
        setViewingTaskId(taskId); 
        navigate(`/orders/form?id=${taskId}`);
                            ...
                            <div key={task.id} className=" bg-green-500 text-white p-4 rounded-lg shadow-lg flex items-center justify-between">
                                <div>
                                    ...
                                    <div className="mt-2 flex items-center space-x-2">
                                        <span
                                            className={`px-2 py-1 text-xs font-medium rounded-full ${task.urgency === "High" ? "bg-red-100 text-red-600" : "bg-yellow-100 text-yellow-600"
                                                }`}
                                        >
                                            {task.urgency} Priority
                                        </span>
                                        <span
                                            className={`px-2 py-1 text-xs font-medium rounded-full ${task.status === "3" ? "bg-blue-100 text-blue-600" : "bg-gray-200 text-gray-600"
                                                }`}
                                        >
            ...
            {viewingTaskId && (
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                    ...
                    <div>
                        ...
                        <button
                            className="bg-sky-500 text-white px-2 ml-2 py-1 rounded"
                            onClick={() => handleOrder(viewingTaskId.jobId)}
                            disabled={viewingTaskId.status === '2'}
                        >
            ...
            {isConfirmModalOpen && (
                <Modal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)}>
                    ...
                    <div>
                        ...
                        <input type="number" name="salary" value={confirmData.salary} onChange={handleCancelDataChange} className="border p-2 w-full" />
                    </div>
                    <div>
