...
export default function UpdateForm() {
  ...
  const generateHTML = (formData, imageData) => {
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; width: 800px; margin: auto;">
        <h1 style="text-align: center;">LOMI TRADING PLC</h1>
        <h2 style="text-align: center;">HVAC Services</h2>
        <p style="text-align: center;">Reference Number: ${formData.reference || ''}</p>
        <h3 style="text-align: center; margin-bottom: 20px;">Job Order and Validation Sheet</h3>
        
        <section>
          <h4 style="border-bottom: 1px solid #000; padding-bottom: 5px;">General Information</h4>
          <table style="width: 100%; margin-bottom: 20px;">
            <tr>
              <td><strong>Job Assessment No:</strong></td>
              <td>${formData.jobAssessmentNo || ''}</td>
            ...
            <tr>
              ...
              <td colspan="3">
                <img src="${imageData.signatureFile4 || ''}" alt="Supervisor Signature" style="max-width: 200px;"/>
              </td>
            </tr>
            <tr>
        ...
        <section>
          <h4 style="border-bottom: 1px solid #000; padding-bottom: 5px;">Problem Details</h4>
          <table style="width: 100%; margin-bottom: 20px;">
            <tr>
              <td><strong>Nature of Problem:</strong></td>
              <td>${formData.natureOfProblem || ''}</td>
            </tr>
            <tr>
              <td><strong>Detailed Problem Reported:</strong></td>
              <td>${formData.detailProblemReported || ''}</td>
            </tr>
            <tr>
              <td><strong>Service Rendered:</strong></td>
              <td>${formData.serviceRendered || ''}</td>
            </tr>
            <tr>
              <td><strong>Customer Comments:</strong></td>
              <td>${formData.customerComment || ''}</td>
            </tr>
          </table>
        </section>
  
        <section>
          <h4 style="border-bottom: 1px solid #000; padding-bottom: 5px;">Materials and Spare Parts Used</h4>
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <thead>
              <tr>
                <th style="border: 1px solid #000; padding: 5px;">S.No</th>
                <th style="border: 1px solid #000; padding: 5px;">Material List</th>
                <th style="border: 1px solid #000; padding: 5px;">MST</th>
            ...
            <tbody>
              ${formData.materials.map((item, index) => `
                <tr>
                  ...
                  <td style="border: 1px solid #000; padding: 5px;">${item.materialUsed || ''}</td>
                  <td style="border: 1px solid #000; padding: 5px;">${item.mst || ''}</td>
                  <td style="border: 1px solid #000; padding: 5px;">${item.qty || ''}</td>
        ...
        <section>
          <h4 style="border-bottom: 1px solid #000; padding-bottom: 5px;">Signatures</h4>
          <table style="width: 100%; margin-bottom: 20px;">
            <tr>
              <td><strong>Executed By:</strong></td>
              <td>${formData.executedBy || ''}</td>
              <td><strong>Position:</strong></td>
              <td>${formData.executedByPosition || ''}</td>
            </tr>
            <tr>
              <td><strong>Signature:</strong></td>
              <td colspan="3">
                <img src="${imageData.signatureFile1 || ''}" alt="Executed By Signature" style="max-width: 200px;"/>
              </td>
            </tr>
            <tr>
              <td><strong>Checked By:</strong></td>
              <td>${formData.checkedBy || ''}</td>
              <td><strong>Position:</strong></td>
              <td>${formData.checkedByPosition || ''}</td>
            </tr>
            <tr>
              <td><strong>Signature:</strong></td>
              <td colspan="3">
                <img src="${imageData.signatureFile2 || ''}" alt="Checked By Signature" style="max-width: 200px;"/>
              </td>
            </tr>
            <tr>
              ...
              <td><strong>Position:</strong></td>
              <td>${formData.approvedByPosition || ''}</td>
            </tr>
            <tr>
              <td><strong>Signature:</strong></td>
              <td colspan="3">
                <img src="${imageData.signatureFile3 || ''}" alt="Approved By Signature" style="max-width: 200px;"/>
              </td>
            </tr>
          </table>
        </section>
  ...
  const getUsersData = async (ev) => {
    ...
    ev.preventDefault();
    axiosClient.get('/users-registration')
      .then(({ data }) => {
        setLoading(false)
        setUsers(data.data)
      })
  ...
  const handleSelectChange = (event) => {
    ...
    console.log(userInfo); // Check the URL in the console
    setUrl(buttonUrl);
  ...
  const sendToTelegram = async (pdfBlob, chat_id) => {
  ...
  async function handleSubmit(userData, chat_id) {
    const files = imageUrls
    ...
    try {
      ...
      await sendToTelegram(pdfBlob, chat_id);
  ...
  const handleApprove = async () => {
    if (!isChecked) {
      alert("Please check the box before approving.");
      return;
    }

    try {
      const response = await axiosClient.put(`/approve/${id}`, { approved: 1 }, {
      ...
      <div className="container">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Approve Button */}
            <button
              className={`px-4 py-2 text-white rounded ${isChecked ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-400 cursor-not-allowed"
                }`}
        ...
        {previews.signatureFile4 ? (
          ...
          imageUrls.signatureFile4 && <img src={imageUrls.signatureFile4} alt="Existing 4" style={{ maxWidth: '300px', marginTop: '10px' }} />
              ...
              <div className="mb-3 flex items-center space-x-4 mt-5">
                ...
                <button
                  className="w-30 block bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded shadow-lg transition duration-200 ease-in-out text-center"

                  onClick={() => generateHTML(formData, imageUrls)}>
                  Print
                </button>
            ...
            <div className="relative bg-white p-6 rounded-lg shadow-lg w-full max-w-lg max-h-[90vh] overflow-auto">
              ...
              {selectedUser && (
                ...
                <><div className="mt-4">
                  ...
                  className="w-full block bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded shadow-lg transition duration-200 ease-in-out text-center"
                  type="button"
                  onClick={() => handleSubmit(formData, selectedUser.chat_id)}
                  disabled={isLoading}
                >
                    Send
                  </button></>
