  ...
  if (formData.materials && formData.materials.length > 0) {
    ...
    try {
        ...
        if (id) {
            ...
            navigate(`/orders/${id}`); // Navigate after update if needed
        } else {
            // Create new user
            ...
            // Assuming the backend returns the new user's ID
            const userId = response.data.data.id;
    setSuccess(response.data.message)
            // Navigate to the new route with the user ID
...
<div className="form-row">
        ...
        <input type="number" required id="job_id" name="job_id" value={formData.job_id} onChange={handleChange} />
    </div>
    <div className="form-row">
