...
const createUser = async (users) => {
    ...
    try {
      ...
      await connection.execute(query, [firstName, lastName, email, phone, password, chat_id, username, role, status, availability, skillsJson]);
...
async function getUserById(userId) {
    try {
        ...
        rows.forEach(row => {
          if (row.job_id) {
            user.jobs.push({
              ...
              technicians: row.technician_id ? [{ technician_id: row.technician_id }] : []
            });
          }
        });
      ...
      throw error; // Optionally re-throw the error to be handled by caller
    }
  }
